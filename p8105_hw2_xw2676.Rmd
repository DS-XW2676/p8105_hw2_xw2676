...
#### Interpretation
There are `r nrow(pols_month)` observations and `r ncol(pols_month)` variables in pols_month dataset. This dataset is the monthly data of political parties of states, politicians and presidents in USA. The key variables are `r colnames(pols_month[2:8])`. 

There are `r nrow(snp)` observations and `r ncol(snp)` variables in snp dataset. The key variables are `r colnames(snp[2])`. 

There are `r nrow(unemployment)` observations and `r ncol(unemployment)` variables in unemployment dataset. This dataset is the monthly data of  unemployment percentage.The key variables are `r colnames(unemployment[2])`. 
